...
usage() {
  echo "Usage: $0 --item-name <1password-item-name> [--role-arn <role-arn>] [--mfa-serial <mfa-serial>] [--mfa-item-name <mfa-item-name>] [--cache-key <cache-key>]"
  exit 1
}
...
# --- Cache Logic ---
if [ -n "$CACHE_KEY" ]; then
  CACHE_FILE="$XDG_RUNTIME_DIR/aws_credential_helper_cache_$CACHE_KEY"
